...
<form name="search1" method="post" action="">
  <table width="100%" cellpadding="1" cellspacing="1">
  	<tr>
		<td colspan="5"><h2 style="color:#003162;">Appointment History</h2></td>
	</tr>
	<tr>
		<td colspan="5">&nbsp;</td>
	</tr>  
	...
	$sel_booking=mysql_query("select * from booking,booking_details where booking.BOOKING_ID=booking_details.BOOKING_ID and DOCTOR_ID='".$_SESSION['doc_id']."' and APT_DATE='".date('Y-m-d')."' and APT_TIME < '".$present_slot_show->SLOT_ID."' and AP_FLAG=1 ");
    ...
    <tr>
	  ...
	  <? } else { ?>
	  <a href="add_precription.php?patient_id=<?php print $val->PATIENT_ID; ?>&fname=<?=$val->FNAME?>&lname=<?=$val->LNAME?>&reason=<?=$val->APT_REASON?>&bk_id=<?=$val->BOOKING_ID?>" class="iframe"><img src="images/prescription.png" title="Add Prescription" style="cursor:pointer;" /></a>
	  <? } ?>
	  
	  <? if($sel_vital>0) { ?>
	  	<img src="images/vitals.png" title="Vital Statistics already added" />
	  <? } else { ?>
	  &nbsp;&nbsp;&nbsp;<a href="add_vitals.php?patient_id=<?php print $val->PATIENT_ID; ?>&fname=<?=$val->FNAME?>&lname=<?=$val->LNAME?>&reason=<?=$val->APT_REASON?>&bk_id=<?=$val->BOOKING_ID?>" class="iframe"><img src="images/vitals.png" title="Add Vital Statistics" style="cursor:pointer;" /></a>
	  <? } ?>
	  
	  
	  &nbsp;&nbsp;&nbsp;<a href="refer.php?patient_id=<?php print $val->PATIENT_ID; ?>&fname=<?=$val->FNAME?>&lname=<?=$val->LNAME?>&reason=<?=$val->APT_REASON?>"><img src="images/referrel.png" title="Reffer" style="cursor:pointer;"  /></a></td>
    </tr>
    <?php 
	$i=$i+1;
	}
	...
	$sel_booking1=mysql_query("select * from booking,booking_details where booking.BOOKING_ID=booking_details.BOOKING_ID and DOCTOR_ID='".$_SESSION['doc_id']."' and APT_DATE='".$apt_date1."' and AP_FLAG=1 ");
	?>
	<tr>
    ...
    <tr> 
	  ...
	  <img src="images/prescription.png" title="Prescription already added" />
	  <? } else { ?>
	  <a href="add_precription.php?patient_id=<?php print $val1->PATIENT_ID; ?>&fname=<?=$val1->FNAME?>&lname=<?=$val1->LNAME?>&reason=<?=$val1->APT_REASON?>&bk_id=<?=$val1->BOOKING_ID?>" class="iframe"><img src="images/prescription.png" title="Add Prescription" style="cursor:pointer;" /></a>
	  <? } ?>
	  
	  <? if($sel_vital1>0) { ?> 
	  	<img src="images/vitals.png" title="Vital Statistics already added" />
	  <? } else { ?>
	  &nbsp;&nbsp;&nbsp;<a href="add_vitals.php?patient_id=<?php print $val1->PATIENT_ID; ?>&fname=<?=$val1->FNAME?>&lname=<?=$val1->LNAME?>&reason=<?=$val1->APT_REASON?>&bk_id=<?=$val1->BOOKING_ID?>" class="iframe"><img src="images/vitals.png" title="Add Vital Statistics" style="cursor:pointer;" /></a>
	  <? } ?>
	  
	  &nbsp;&nbsp;&nbsp;<a href="refer.php?patient_id=<?php print $val1->PATIENT_ID; ?>&fname=<?=$val1->FNAME?>&lname=<?=$val1->LNAME?>&reason=<?=$val1->APT_REASON?>"><img src="images/referrel.png" title="Reffer" style="cursor:pointer;"  /></a>
