...
if($_SESSION['username'])
{
	...
	$query="SELECT * from patientregister where PATIENT_USERNAME='".$_SESSION['username']."'";
	...
	$pat_val=mysql_fetch_object(mysql_query("select * from patient_personal_info where PATIENT_ID='$rowid'"));
	if($pat_val->PATIENT_PHOTO!='')
	{
		$img_name=$pat_val->PATIENT_PHOTO;
	}
	else
	{
		$img_name='noImage.gif';
	}
	if($pat_val->PATIENT_DOB!='')
	{
	...
	<table width="720" cellpadding="0" cellspacing="0" border="0">
		...
		<tr>
          ...
          <tr>
            ...
            <tr>
              <td colspan="3">
				<input  type="text" size="50" valign="top"  name="dob"  value="<?php print $user_dob; ?>" onClick="scwShow(this,event);" id = "dob"  maxlength="15" class="textfild-LS" style="width:326px;"/>
				&nbsp;&nbsp;</td>
            </tr>
            <tr>
              <td colspan="3">&nbsp;</td>
			...
			<tr>
              <td colspan="3"><select name="gender" id="gender" class="drop3">
				...
				<option value="Male" <?php if($pat_val->PATIENT_GENDER=='Male') { ?> selected="selected" <?php } ?> >Male</option>
				<option value="Female" <?php if($pat_val->PATIENT_GENDER=='Female') { ?> selected="selected" <?php } ?> >Female</option>
			</select></td>
            </tr>			
			<tr>
              <td colspan="3">&nbsp;</td>
		  ...
		  <tr>
			<td colspan="3"><input type="text" class="textfiled-M" name="address" id="address" value="<?php print $pat_val->PATIENT_ADDRESS;?>" maxlength="200" /></td>
		  </tr>
		  <tr>
		  <td colspan="3">&nbsp;</td>
		  </tr>
          ...
          <tr>
              <td colspan="3">
				...
				<option value="" <?php if($pat_val->PATIENT_CONTRY==""){?>selected="selected" <?php }?>>Select Country</option>
					<?php 
					$sel_query=mysql_query("select * from country_table");
					while($row1=mysql_fetch_object($sel_query)) { 
					?>
					<option value="<?php print $row1->COUNTRY_ID; ?>"<?php if($pat_val->PATIENT_CONTRY==$row1->COUNTRY_ID) { ?> selected="selected" <?php } ?>><?php print $row1->COUNTRY_NAME; ?></option>
					<?php } ?>
				</select></td>
            </tr>
			<tr>
            ...
            <tr>
              <td colspan="3">
				...
				<option value="" <?php if($pat_val->PATIENT_STATE==""){?>selected="selected"<?php } ?>>Select</option>
			<?php 
					$sel_query=mysql_query("select * from state_table where COUNTRY_ID='".$pat_val->PATIENT_CONTRY."'");
					while($row2=mysql_fetch_object($sel_query)) { 
					?>
					<option value="<?=$row2->STATE_ID?>" <?php if($pat_val->PATIENT_STATE==$row2->STATE_ID) { ?> selected="selected" <?php } ?>><?php print $row2->STATE_NAME; ?></option>
					<?php } ?>
			</select></div></td>
            </tr>
			<tr>
            ...
            <tr>
              <td colspan="3">
				...
				<option value="" <?php if($pat_val->PATIENT_CITY==""){?> selected="selected"<?php } ?>>Select</option>
				<?php 
					$sel_query=mysql_query("select * from city_table where STATE_ID='".$pat_val->PATIENT_STATE."'");
					while($row3=mysql_fetch_object($sel_query)) { 
					?>
					<option value="<?=$row3->CITY_ID?>" <?php if($pat_val->PATIENT_CITY==$row3->CITY_ID) { ?> selected="selected" <?php } ?>><?php print $row3->CITY_NAME; ?></option>
					<?php } ?>				
				</select></div></td>
            </tr>
			<tr>
            ...
            <tr>
              <td colspan="3">
				...
				<option value="" <?php if($pat_val->PATIENT_AREA==""){?> selected="selected"<?php }?>>Select</option>	
					<?php 
					$sel_query=mysql_query("select * from area_table where CITY_ID='".$pat_val->PATIENT_CITY."'");
					while($row4=mysql_fetch_object($sel_query)) { 
					?>
					<option value="<?php print $row4->AREA_ID; ?>" <?php if($pat_val->PATIENT_AREA==$row4->AREA_ID) { ?> selected="selected" <?php } ?>><?php print $row4->AREA_NAME; ?></option>
					<?php } ?>				
				</select></div></td>
            </tr>
			<tr>
...
<?php 
	if($_SERVER['REQUEST_METHOD']=='POST')
	{
	  ...
	  if($dob>$origin_date)
	  {
		...
		$rowcount=mysql_num_rows(mysql_query("select * from patient_personal_info where PATIENT_ID='".$rowid."'"));
		if($rowcount>=1)
		{
			 if($_FILES["file"]["name"]!='')
			 {
			...
			$reg_info=mysql_query("update patientregister set PATIENT_FIRST_NAME='".$_POST['fname']."', PATIENT_MIDDLE_NAME='".$_POST['mname']."', PATIENT_LAST_NAME='".$_POST['lname']."',PATIENT_MOBILE_NUMBER='".$_POST['mobile']."',PATIENT_EMAIL='".$_POST['email']."' where PATIENT_ID='".$rowid."'");
		
			$personal_info=mysql_query("update patient_personal_info set PATIENT_DOB='".$dob."', PATIENT_GENDER='".$_POST['gender']."', PATIENT_ADDRESS='".$_POST['address']."', PATIENT_CONTRY='".$_POST['country1']."', PATIENT_STATE='".$_REQUEST['cmbState']."', PATIENT_CITY='".$_REQUEST['cmbCity']."', PATIENT_AREA='".$_REQUEST['cmbArea']."', PATIENT_PHOTO='".$photo."', PATIENT_RECORD_DATE='".date('Y-m-d')."' where PATIENT_ID='".$rowid."'"); 
			
			if($reg_info==1 && $personal_info==1)
			{
				print("<script language='javascript'>alert('Request Completed...'); window.location.href='update_profile_p.php';</script>");
			}
			else
			{
				print("<script language='javascript'>alert('Request Incomplete, Try Again...'); window.location.href='update_profile_p.php';</script>");	
			}
		}
		else
		{
			...
			$personal_info=mysql_query("insert into patient_personal_info (PATIENT_ID,PATIENT_DOB,PATIENT_GENDER,PATIENT_ADDRESS,PATIENT_CONTRY,PATIENT_STATE,PATIENT_CITY,PATIENT_AREA,PATIENT_PHOTO,PATIENT_RECORD_DATE) values('".$rowid."','".$dob."','".$_POST['gender']."','".$_POST['address']."','".$_POST['country1']."','".$_REQUEST['cmbState']."','".$_REQUEST['cmbCity']."','".$_REQUEST['cmbArea']."','".$photo."','".date('Y-m-d')."')");
			
			if($reg_info==1 && $personal_info==1)
			{
				/*print("<script language='javascript'>alert('Request Completed...'); window.location.href='update_profile_p.php';</script>");*/
				print("<script language='javascript'>alert('Request Completed...'); window.parent.location='dashboard.php';</script>");
			}
			else
			{
				print("<script language='javascript'>alert('Request Incomplete, Try Again...'); window.location.href='update_profile_p.php';</script>");	
