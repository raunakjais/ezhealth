...
if($_SESSION['username'])
{
	...
	if($_GET['id']=='limit')
	{
		$query=("SELECT * from business_info,login where BUSINESS_ID=USER_UN_ID and USER_TYPE='B' and USER_FLAG='1' and BUSINESS_RECORD_DATE >='".$site_admin->cdate."' ");
	}
	else
	{
		$query=("SELECT * from business_info,login where BUSINESS_ID=USER_UN_ID and USER_TYPE='B' and USER_FLAG='1' ");
	}
...
$(function() 
{
	...
	if(post_val3==''){ alert('please enter remark'); return false; }
	 ...
	 var dataString = 'buss_id='+ ID +'&post_val1='+ post_val1 +'&post_val2='+ post_val2 +'&post_val3='+ post_val3;
	 
	if(confirm("Sure you want to update payment!"))
	{
	$.ajax({
	type: "POST",
	 url: "update_data_b.php",
	  data: dataString,
	 cache: false,
...
<tr><td colspan="5"><h2 style="color:#003162;">Business Approval</h2></td></tr>
<tr>
<td colspan="5" height="6"></td>
</tr>
<tr><td colspan="5"><div align="center" id="manoj" style="display:none; background-color:#33CC33;"><?php echo $message; ?>dfhsgdsfg</div>
</td></tr>
<tr>
...
<tr>
	<td colspan="5"><hr /></td>
		  ...
		  ?>
<div class="bar<? print $row['BUSINESS_ID']?>">		  
	<tr>
	<td>
<?php print $businessname=$row['BUSINESS_LOCATION_TITLE']." ".$row['BUSINESS_LOCATION_TYPE'];?>
<input type="hidden" name="buss_id" id="buss_id" value="<?php print $row['BUSINESS_ID']; ?>" />
...
<td><input type="text" name="remark_<? print $i; ?>" id="remark_<? print $i; ?>" onKeyUp="same3(this.value);" class="textfild-S1" value=""/>

</td>
<td><img src="images/health-icon.png" id="<?php print $row['BUSINESS_ID']; ?>" class="update_btn" /></td>
</tr>		 
  </div>		<? 
		$i=$i+1;
		   }
		   ...
		   <tr>
		   <td align="right" colspan="5"><a href="user_approval_b.php" style="text-decoration:none; color:#333333;"><strong>View All</strong></a></td>
		   </tr>
		   <?php } ?>      
</table>
</form>
