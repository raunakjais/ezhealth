...
<table width="720" cellpadding="1" cellspacing="1" border="0"> 
		  ...
		  <?php 
		  $get_doc = mysql_query("select * from doc_mapping where BUSS_ID='".$get_bid->USER_UN_ID."'");
		  ...
		  <tr>
			...
			<td class="timetabH1">Yes</td>
			<td class="timetabH2"><a href="view_mapping.php?del_id=<?php print $doc_det->DOC_MAPPING_ID; ?>" style="text-decoration:none; color:#FF0000;"><strong>DELETE</strong></a> </td>
		  </tr>
		  <?php } ?>
        </table>
</form>
...
<?php
	if($_GET['del_id']!='')
	{
		$delete_map = mysql_query("delete from doc_mapping where DOC_MAPPING_ID='".$_GET['del_id']."'");
		if($delete_map==1)
		{
			print("<script language='javascript'>alert('mapping deleted successfully'); window.location.href='view_mapping.php';</script>");
		}
	}
		/*if($_GET['do']=='act')
		{
			...
			if($update_map==1)
			{
				print("<script language='javascript'>alert('mapping activated successfully'); window.location.href='view_mapping.php';</script>");
			}
		}*/
}
else
