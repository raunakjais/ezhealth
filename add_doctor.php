...
<table width="700" cellpadding="0" cellspacing="0" border="0"> 
			<tr>
				<td colspan="3"><h2 style="color:#003162;">Add Doctor</h2></td>
			</tr>
		  <tr>
            <td colspan="3">&nbsp;</td>
          </tr>        
...
<?php
	if(($_POST['submit'])=='Submit')
	{
		...
		if($_REQUEST['mname']!='')
		{
		$disp_name=$_REQUEST['fname'].' '.$_REQUEST['mname'].' '.$_REQUEST['lname'];
		}
		else
		{
		$disp_name=$_REQUEST['fname'].' '.$_REQUEST['lname'];
		}
		$insert_query=mysql_query("insert into doctor_register (DOCTOR_FIRST_NAME,DOCTOR_MIDDLE_NAME,DOCTOR_LAST_NAME,DOCTOR_DISPLAY_NAME,DOCTOR_MOBILE_NUMBER,DOCTOR_EMAIL,DOCTOR_USERNAME,DOCTOR_PASSWORD,DOCTOR_IDENTITY,DOCTOR_RECORD_DATE,DOCTOR_FLAG) values('".$_POST['fname']."','".$_POST['mname']."','".$_POST['lname']."','".$disp_name."','".$_POST['mobile']."','".$_POST['email']."','".$_POST['username']."','".$_POST['pwd']."','".$get_bid->USER_UN_ID."','".date('Y-m-d')."','0')");
		...
		if($insert_query==1)
		{
			...
			if($insert_login==1)
			{
				...
				print("<script language='javascript'>alert('doctor added successfully'); window.location.href='add_doctor.php';</script>");
			}	
		}	
