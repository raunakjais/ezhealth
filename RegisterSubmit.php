...
<script type="text/javascript" src="resend.js"></script>
...
function send_code()
{
	resend(document.getElementById('u_name').value+'^'+document.getElementById('rno').value+'^'+document.getElementById('u_email').value,'disp_msg')
}
</script>
</head>
...
<div class="midcontainer">
		...
		<table width="720" cellpadding="0" cellspacing="0" border="0">          
          ...
          <tr>
            <td colspan="3"><input type="text" class="textfiled-M" size="25" name="regcode" value="" id = "regcode"  maxlength="15"  />&nbsp; &nbsp;<img src="images/resend.png" style="padding-top:3px; cursor:pointer;" class="update_btn1" title="Resend Code Again" onclick="send_code();" /> <div align="center" id="disp_msg" ><?php echo $message; ?></div></td>
          </tr>
          <tr>
            <td colspan="3">&nbsp;</td>
          </tr>
	...
	$num=mysql_num_rows($row);
		if($num>=1)
		{
			if($_SESSION['user_type']=='D')
			{
				...
				if($doc_reg->DOCTOR_MIDDLE_NAME!='')
				{
				$disp_name=$doc_reg->DOCTOR_FIRST_NAME.' '.$doc_reg->DOCTOR_MIDDLE_NAME.' '.$doc_reg->DOCTOR_LAST_NAME;
				}
				else
				{
				$disp_name=$doc_reg->DOCTOR_FIRST_NAME.' '.$doc_reg->DOCTOR_LAST_NAME;
				}
				$insert_doc_register=mysql_query("insert into doctor_register (DOCTOR_FIRST_NAME,DOCTOR_MIDDLE_NAME,DOCTOR_LAST_NAME,DOCTOR_DISPLAY_NAME,DOCTOR_MOBILE_NUMBER,DOCTOR_EMAIL,DOCTOR_USERNAME,DOCTOR_PASSWORD,DOCTOR_RECORD_DATE,DOCTOR_FLAG) values ('".$doc_reg->DOCTOR_FIRST_NAME."','".$doc_reg->DOCTOR_MIDDLE_NAME."','".$doc_reg->DOCTOR_LAST_NAME."','".$disp_name."','".$doc_reg->DOCTOR_MOBILE_NUMBER."','".$doc_reg->DOCTOR_EMAIL."','".$doc_reg->DOCTOR_USERNAME."','".mysql_real_escape_string($doc_reg->DOCTOR_PASSWORD)."','".$doc_reg->DOCTOR_RECORD_DATE."','".$doc_reg->DOCTOR_FLAG."')");
				...
				if($insert_doc_register==1)
				{
					...
					$insert_login=mysql_query("insert into login (USER_UN_ID,USER_TYPE,USERNAME,USER_EMAIL,USER_PASSWORD,USER_FLAG) values('".$user_un_id."','D','".$uname."','".$email."','".mysql_real_escape_string($doc_login->USER_PASSWORD)."','1')");	
					if($insert_login==1)
					{
			...
			else if($_SESSION['user_type']=='B')
			{
				...
				if($insert_buss_register==1)
				{
					...
					$insert_login=mysql_query("insert into login (USER_UN_ID,USER_TYPE,USERNAME,USER_EMAIL,USER_PASSWORD,USER_FLAG) values('".$user_un_id."','B','".$uname."','".$email."','".mysql_real_escape_string($buss_login->USER_PASSWORD)."','1')");	
					if($insert_login==1)
					{
						$ch = curl_init();
						curl_setopt($ch, CURLOPT_URL,"http://myvaluefirst.com/smpp/sendsms?");
			...
			else
			{
				...
				if($insert_patient_register==1)
				{
					...
					$insert_login=mysql_query("insert into login (USER_UN_ID,USER_TYPE,USERNAME,USER_EMAIL,USER_PASSWORD,USER_FLAG) values('".$user_un_id."','P','".$uname."','".$email."','".mysql_real_escape_string($pat_login->USER_PASSWORD)."','0')");	
					if($insert_login==1)
					{
						$ch = curl_init();
						curl_setopt($ch, CURLOPT_URL,"http://myvaluefirst.com/smpp/sendsms?");
						...
						$del_temp_login=mysql_query("delete from login_temp where USER_EMAIL='".$email."' and USERNAME='".$uname."'");
						if($_SESSION['new_doc_id']=='' && $_SESSION['doc_id']=='')
						{
						print("<script language='javascript'>alert('Your account has been created, please login to access your account.');window.location.href='index.php';</script>");
						} 
						else
