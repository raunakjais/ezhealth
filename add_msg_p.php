...
/*function showLoc()
{
	...
	else if(document.getElementById('userType').value=='HOSPITAL')
	{
		document.getElementById('doc').style.display='none';
...
function validate()
{
	...
	else if(document.getElementById('consent').value=='')
	{
...
<tr>
	<td colspan="7"><h2 style="color:#003162;">Add Message</h2></td>
	<td colspan="1"><a href="message_patient.php" target="riframe"><img src='./images/close.png' border="0" title="Close"></a></td>
</tr>
<tr>
	<td colspan="8">&nbsp;</td>
</tr>
...
<tr>
	<td colspan="8">
		<select class="drop2" name="doctor" id="doctor">
			...
			<?php $sel_doc=mysql_query("select distinct DOCTOR_ID from booking where PATIENT_ID='".$_SESSION['pat_id']."'");
			...
			?>
				<option value="<?=$val->DOCTOR_ID?>"><?=$doc_name->DOCTOR_FIRST_NAME.' '.$doc_name->DOCTOR_LAST_NAME?></option>
			<? } ?>
		</select>
	</td>
</tr>
<tr>
	<td colspan="8">Consent <span class="red">*</span></td>
</tr>
<tr>
	<td colspan="8">
...
<?php
	if($_SERVER['REQUEST_METHOD']=='POST')
	{
		...
		{*/
			...
			$TO_TYPE='D';
		/*}
		else if($_REQUEST['userType']=='HOSPITAL')
		{
			$TO_ID=$_REQUEST['hospital'];
			$TO_TYPE='H';
		}*/
		$insert_query=mysql_query("insert into chat (SUBJECT,FROM_ID,FROM_TYPE,TO_ID,TO_TYPE,CONSENT,CUSER,CDATE) values ('".$_REQUEST['subject']."','".$_SESSION['pat_id']."','".$_SESSION['user_type']."','".$TO_ID."','".$TO_TYPE."','".$_REQUEST['consent']."','".$_SESSION['username']."','".date('Y-m-d')."')");
		$chat_id=mysql_insert_id();
		if($insert_query==1)
		{
			$insert_chat=mysql_query("insert into chat_details (CHAT_ID,CHAT_BY,TXT_BY,CHAT_DETAILS,CDATE) values ('".$chat_id."','".$_SESSION['user_type']."','".$_SESSION['pat_id']."','".$_REQUEST['desc']."','".date('Y-m-d')."') ");
			if($insert_chat==1)
			{
